...
@section('content')
  ...
  <div
    class="mx-auto max-w-wide px-4 py-16 md:px-8 lg:py-24"
    x-data="{ tab: window.location.hash === '#past' ? 'past' : 'upcoming' }"
    x-init="$watch('tab', (v) => history.replaceState(null, '', v === 'past' ? '#past' : location.pathname + location.search))"
  >
    ...
    <div role="region" aria-label="{{ __('Upcoming events', 'sage') }}" x-show="tab === 'upcoming'">
      @if ($upcoming->have_posts())
        <div class="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
          @while ($upcoming->have_posts()) @php($upcoming->the_post())
            @include('partials.content-event')
          @endwhile
          @php(wp_reset_postdata())
        </div>
      @else
        <p class="text-lg text-gray">{{ __('There are no upcoming events right now — check back soon.', 'sage') }}</p>
      @endif
    </div>
    ...
    <div role="region" aria-label="{{ __('Past events', 'sage') }}" x-show="tab === 'past'" x-cloak>
      @if ($past->have_posts())
        <div class="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
          @while ($past->have_posts()) @php($past->the_post())
            @include('partials.content-event')
          @endwhile
          @php(wp_reset_postdata())
        </div>
      @else
        <p class="text-lg text-gray">{{ __('There are no past events to show yet.', 'sage') }}</p>
      @endif
    </div>
  </div>
@endsection
