                ...
                @else
                    <div x-cloak x-data="{
                        ...
                        <div x-show="showControls" class="mt-8 flex justify-center gap-3">
                            <button type="button" aria-label="{{ __('Previous slides', 'sage') }}"
                                class="flex size-10 items-center justify-center rounded-full bg-pink text-white transition hover:bg-red"
                                x-bind:class="{ 'opacity-25': !showPreviousControl }"
                                x-bind:disabled="!showPreviousControl" @click="swiper.slidePrev()">
                                @svg('chevron-left', 'size-5')
                            </button>

                            <button type="button" aria-label="{{ __('Next slides', 'sage') }}"
                                class="flex size-10 items-center justify-center rounded-full bg-pink text-white transition hover:bg-red"
                                x-bind:class="{ 'opacity-25': !showNextControl }" x-bind:disabled="!showNextControl"
