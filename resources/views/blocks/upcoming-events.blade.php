@unless ($block->preview)
    ...
    <section class="wp-block alignfull relative overflow-x-clip bg-cream ">
        <div class="mx-auto grid max-w-wide items-center gap-10 px-4 lg:grid-cols-2">
                            ...
                            <li>
                                ...
                                @if ($start = get_field('start', $event->ID))
                                    <time datetime="{{ date('c', strtotime($start)) }}"
                                        class="mt-1 block text-sm font-bold">{{ wp_date('D jS M', strtotime($start)) }}</time>
                                @endif
