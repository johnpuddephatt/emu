@unless ($block->preview)
    ...
    @if ($block->style === 'rows')
        <section class="wp-block alignfull relative py-16 lg:py-24">
            <div class="mx-auto max-w-wide px-4">
                <div class="mx-auto prose max-w-content text-center">
    ...
    @else
        <section class="wp-block alignfull relative has-black-background-color py-16 lg:py-24">
            <div class="mx-auto max-w-wide px-4">
                <div class="mx-auto prose  prose-invert max-w-content text-center">
                    <InnerBlocks template="{{ $block->template }}" />
                </div>
                ...
                @if ($cards)
                    <div class="relative mt-12">
                        <ul
                            class="relative grid list-none gap-4 lg:gap-6 p-0 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                            @foreach ($cards as $card)
                                <li class="  flex relative flex-col group rounded-xl bg-white p-5 text-black">
                                    <div aria-hidden="true"
                                        class="pointer-events-none absolute group-hover:rotate-0 transition rotate-3 rounded-xl border-2 border-yellow inset-0">
                                    </div>

                                    @if ($card['illustration'])
                                        {!! wp_get_attachment_image($card['illustration'], 'medium', false, [
                                            'class' => 'mx-auto h-36 w-auto object-contain',
                                        ]) !!}
                                    @endif
