            ...
            @if ($cards)
                <ul class="mx-auto mt-12 flex max-w-6xl list-none flex-col gap-8 p-0">
                    @foreach ($cards as $card)
                        <li class="flex flex-col gap-6 rounded-xl border-2 border-yellow-dark bg-white p-6 text-black sm:p-10 lg:items-center lg:gap-16 {{ $loop->even ? 'lg:flex-row-reverse' : 'lg:flex-row' }}">
                            <div class="flex-1">
        ...
        <div class="max-w-wide mx-auto px-4">
            ...
            @if ($cards)
                <div class="relative mt-12">
                    <ul class="relative grid list-none gap-4 p-0 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6 xl:grid-cols-5">
                        @foreach ($cards as $card)
                            <li class="group relative flex flex-col rounded-xl bg-white p-5 text-black">
                                <div
                                    aria-hidden="true"
