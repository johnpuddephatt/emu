    ...
    @while (have_posts())
        ...
        <article @php(post_class('h-entry'))>
            <header class="relative overflow-x-clip has-black-background-color">
                <div
                    class="relative mx-auto max-w-content px-4 pt-28 text-center lg:pt-32 {{ has_post_thumbnail() ? 'pb-24 lg:pb-32' : 'pb-16 lg:pb-20' }}">
                    @if ($category = get_the_category()[0] ?? null)
                        <p class="mb-4 text-sm font-bold text-yellow">
                            <a href="{{ get_category_link($category) }}"
                                class="no-underline hover:underline">{{ $category->name }}</a>
                        </p>
                    @endif
                ...
                <a href="{{ home_url('/') }}" class="absolute top-4 left-4 z-30 lg:top-6 lg:left-6"
                    aria-label="{{ $siteName }}">
            ...
            @if (has_post_thumbnail())
                <div class="relative z-10 mx-auto -mt-14 max-w-wide px-4 lg:-mt-20">
